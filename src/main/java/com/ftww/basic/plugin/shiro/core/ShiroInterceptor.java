...
public class ShiroInterceptor implements Interceptor {
	...
	public void intercept(Invocation inv) {
		...
		// 路径权限 //注解权限
		List<AuthzHandler> ahs = ShiroKit.getAuthzHandler(inv.getController().getRequest(), inv.getActionKey());
		// 权限验证
		if (assertNoAuthorized(inv, ahs))
			return;
		// 执行正常逻辑
		inv.invoke();
	}
	 ...
	 * @return
	 */
	private boolean assertNoAuthorized(Invocation inv, List<AuthzHandler> ahs) {
		...
		if (ahs != null && ahs.size() > 0) {
			...
			if (!SubjectKit.isAuthed()) {
				WebUtils.saveRequest(inv.getController().getRequest());
			}
			...
			} catch (UnauthenticatedException lae) {//未登陆验证，所以抛出该异常。
