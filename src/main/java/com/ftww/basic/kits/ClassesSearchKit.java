...
/**
 * 修改dream-pie的类检索工具类
 * Created by poicom7 on 2015/8/18.
 */
public class ClassesSearchKit {

    protected static final Logger LOG = Logger.getLogger(ClassesSearchKit.class);
    ...
    public static ClassesSearchKit of(Class target) {
        return new ClassesSearchKit(target);
    }
    ...
    private static List<String> findFiles(String baseDirName, String targetFileName) {
        ...
        try {
            baseURLs = ClassesSearchKit.class.getClassLoader().getResources(File.separator + baseDirName.replaceAll("\\.", "/"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        URL baseURL = null;
...
//	      } else
        ...
        if (!baseDir.exists() || !baseDir.isDirectory()) {
            ...
            for (int i = 0; i < filelist.length; i++) {
                ...
                if (readfile.isDirectory()) {
                    ...
                    if (ClassesSearchKit.wildcardMatch(targetFileName, tempName)) {
    ...
    public ClassesSearchKit(Class target) {
        this.target = target;
    }
...
//	    return this;
    ...
    public ClassesSearchKit includepaths(String... classpaths) {
        if (classpaths != null) {
    ...
    public ClassesSearchKit includepaths(List<String> classpaths) {
        if (classpaths != null) {
            for (String classpath : classpaths) {
                this.includepaths.add(classpath);
            }
    ...
    public <T> List<Class<? extends T>> search() {
        if (includepaths.size() <= 0) {
            ...
            try {
                resources = ClassesSearchKit.class.getClassLoader().getResources("");
            } catch (IOException e) {
                e.printStackTrace();
            }
            URL resource = null;
