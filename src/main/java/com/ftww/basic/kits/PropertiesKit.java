...
public class PropertiesKit {
	...
	public Properties loadPropertyFile(boolean reload, String file) {
		...
		if (startStuff)
			...
			fullFile = PathKit.getWebRootPath() + File.separator + "WEB-INF"
					+ File.separator + file;
		File propFile = new File(fullFile);	//fullFile为文件绝对路径名称。

		// 判断文件是否存在WebInf
		if (!propFile.exists()) {	//	不存在该文件，则查找是否已经存入集合中。
			if (!startStuff) {
				fullFile = "/" + file;
			} else {
				fullFile = file;
		...
		if (properties != null) {	//存在该配置文件，存放文件入集合。
			propertiesFiles.put(fullFile, properties);
			for (Entry<Object, Object> entry : properties.entrySet()) {
				this.propertiesAttrs.put(entry.getKey().toString(),	//	提取配置文件内容，存放内容如集合。
						entry.getValue());
			}
		}
		return properties;
